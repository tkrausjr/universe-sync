...
src_registry_host = '192.168.62.128'
src_registry_port = 5000
src_http_port = 8083
src_insecure = True

# docker_client = docker.DockerClient(base_url='tcp://127.0.0.1:2375',tls=True)
print ("Docker Client Info =" + str(docker_client.info()))
pulled_images =[]
...
def pull_images(registry_host,registry_port,src_manifests,insecure,docker_client):
    '''
    :param repos: Repositories Python List returned from Registry API
    ...
    for image,imagetag in src_manifests.items():
            ...
            print("Full Image ID passed to Docker Pull = " + fullImageId)

            #for line in docker_client.images.pull(str(fullImageId),stream=True):
            #   print(json.dumps(json.loads(line), indent=4))


            for line in docker_client.images.pull(registry_host + ':'+str(registry_port) +'/' + image, tag=imagetag,stream=True,insecure_registry=insecure):
                print(json.dumps(json.loads(line), indent=4))
...
def push_images(new_images,insecure,client):
    for fullImageRef in new_images:
        ...
        for line in client.images.push(fullImageRef,stream=True,insecure_registry=insecure):
            print (line)
...
if __name__ == "__main__":
    ...
    pulled_images = pull_images(src_registry_host,src_registry_port,src_manifests,src_insecure,docker_client)
    ...
    pushed_images = push_images(new_images,dst_insecure,docker_client)
